...
contract MyDAO {
    ...
    event proposal_info(uint256 id, address author, string name, string[] options, uint256[] num_options, uint256 power, uint256 proposal_info_type);
 ...
 /** since we need 25 tokens to vote and one token is 25 wei so 25 * 10^18 shares */
    ...
    function createProposal(string memory name, string[] memory _options, uint256[] memory _options_num, uint256 _power) external {
