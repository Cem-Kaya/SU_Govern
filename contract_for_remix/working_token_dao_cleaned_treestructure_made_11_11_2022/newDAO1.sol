...
contract MyDAO {
    ...
    ERC20 public voter_token;
    ERC20 public yk_token;
    ...
    constructor(string memory _dao_name, string memory _dao_symbol, address first_yk, ERC20 yk_token_in, ERC20 voter_token_in) {
...
//deposits are currently legacy
 ...
 /** since we need 25 tokens to vote and one token is 25 wei so 25 * 10^18 shares */
    ...
    function withdraw_voter(uint _amount) external {
        require(voter_shares_to_be_given[msg.sender] >= _amount, 'Not enough shares');
        voter_shares_to_be_given[msg.sender] -= _amount;
    ...
    function withdraw_yk(uint _amount) external {
        require(yk_shares_to_be_given[msg.sender] >= _amount, 'Not enough YK shares');
        yk_shares_to_be_given[msg.sender] -= _amount;
    ...
    function send_yk_tokens_to_address_yk(address yk_candidate, uint _amount) external {
        require(yk_token.balanceOf(msg.sender) >= _amount * 10 **18, 'Not a YK');
    ...
    //tokens need to be in sender's wallet
    function send_voter_tokens_to_address_voter(address voter_candidate, uint _amount) external {
        require(voter_token.balanceOf(address(msg.sender)) >= _amount* 10 **18, 'Not enough token in wallet');
        //voter_shares[voter_candidate] += (_amount * 10 ** 18);
        //totalShares += (_amount * 10 ** 18);
        //total_voter_shares += (_amount * 10 ** 18);
        voter_token.transferFrom(msg.sender, voter_candidate, (_amount * 10 ** 18));
    ...
    function vote_power_weighted(uint _proposalId, string[] memory _vote, uint[] memory _power) external {
        ...
        uint vote_weight = voter_token.balanceOf(msg.sender) / (10 ** 18); 
